...
//  последовательность упорядоченной по невозрастанию или по неубыванию.
    @Test
    void testEx16() {
        assertEquals("Упорядочена по невозрастанию", testEx16(new int[]{9, 8, 7, 6, 5, 4, 3, 2, 1}));
        assertEquals("Упорядочена по неубыванию", testEx16(new int[]{1, 2, 3, 4, 5, 6, 7, 8, 9}));
        assertEquals("Никак не упорядоченна", testEx16(new int[]{1, 2, 3, 3, 5, 6, 7, 6, 4}));
        assertEquals("Никак не упорядоченна", testEx16(new int[]{1, 2, 3, 3, 3, 6, 7, 8, 9}));
        assertEquals("Упорядочена по неубыванию", testEx16(new int[]{1, 2}));
        assertEquals("Упорядочена по неубыванию", testEx16(new int[]{1, 2, 3, 0, 3, 6, 7, 8, 9}));
    }
